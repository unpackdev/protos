// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.26.0
// 	protoc        v4.23.4
// source: ast/member_access.proto

package ast_pb

import (
	v3 "github.com/cncf/xds/go/xds/type/v3"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type MemberAccessExpression struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Id                    int64              `protobuf:"varint,1,opt,name=id,proto3" json:"id,omitempty"`
	MemberName            string             `protobuf:"bytes,2,opt,name=member_name,json=memberName,proto3" json:"member_name,omitempty"`
	NodeType              NodeType           `protobuf:"varint,3,opt,name=node_type,json=nodeType,proto3,enum=txpull.v1.ast.NodeType" json:"node_type,omitempty"`
	Src                   *Src               `protobuf:"bytes,4,opt,name=src,proto3" json:"src,omitempty"`
	ReferencedDeclaration int64              `protobuf:"varint,5,opt,name=referenced_declaration,json=referencedDeclaration,proto3" json:"referenced_declaration,omitempty"`
	IsConstant            bool               `protobuf:"varint,6,opt,name=is_constant,json=isConstant,proto3" json:"is_constant,omitempty"`
	IsLValue              bool               `protobuf:"varint,7,opt,name=is_l_value,json=isLValue,proto3" json:"is_l_value,omitempty"`
	IsPure                bool               `protobuf:"varint,8,opt,name=is_pure,json=isPure,proto3" json:"is_pure,omitempty"`
	LValueRequested       bool               `protobuf:"varint,9,opt,name=l_value_requested,json=lValueRequested,proto3" json:"l_value_requested,omitempty"`
	Expression            *v3.TypedStruct    `protobuf:"bytes,10,opt,name=expression,proto3" json:"expression,omitempty"`
	ArgumentTypes         []*TypeDescription `protobuf:"bytes,11,rep,name=argument_types,json=argumentTypes,proto3" json:"argument_types,omitempty"`
	TypeDescription       *TypeDescription   `protobuf:"bytes,12,opt,name=type_description,json=typeDescription,proto3" json:"type_description,omitempty"`
}

func (x *MemberAccessExpression) Reset() {
	*x = MemberAccessExpression{}
	if protoimpl.UnsafeEnabled {
		mi := &file_ast_member_access_proto_msgTypes[0]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *MemberAccessExpression) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MemberAccessExpression) ProtoMessage() {}

func (x *MemberAccessExpression) ProtoReflect() protoreflect.Message {
	mi := &file_ast_member_access_proto_msgTypes[0]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MemberAccessExpression.ProtoReflect.Descriptor instead.
func (*MemberAccessExpression) Descriptor() ([]byte, []int) {
	return file_ast_member_access_proto_rawDescGZIP(), []int{0}
}

func (x *MemberAccessExpression) GetId() int64 {
	if x != nil {
		return x.Id
	}
	return 0
}

func (x *MemberAccessExpression) GetMemberName() string {
	if x != nil {
		return x.MemberName
	}
	return ""
}

func (x *MemberAccessExpression) GetNodeType() NodeType {
	if x != nil {
		return x.NodeType
	}
	return NodeType_NT_DEFAULT
}

func (x *MemberAccessExpression) GetSrc() *Src {
	if x != nil {
		return x.Src
	}
	return nil
}

func (x *MemberAccessExpression) GetReferencedDeclaration() int64 {
	if x != nil {
		return x.ReferencedDeclaration
	}
	return 0
}

func (x *MemberAccessExpression) GetIsConstant() bool {
	if x != nil {
		return x.IsConstant
	}
	return false
}

func (x *MemberAccessExpression) GetIsLValue() bool {
	if x != nil {
		return x.IsLValue
	}
	return false
}

func (x *MemberAccessExpression) GetIsPure() bool {
	if x != nil {
		return x.IsPure
	}
	return false
}

func (x *MemberAccessExpression) GetLValueRequested() bool {
	if x != nil {
		return x.LValueRequested
	}
	return false
}

func (x *MemberAccessExpression) GetExpression() *v3.TypedStruct {
	if x != nil {
		return x.Expression
	}
	return nil
}

func (x *MemberAccessExpression) GetArgumentTypes() []*TypeDescription {
	if x != nil {
		return x.ArgumentTypes
	}
	return nil
}

func (x *MemberAccessExpression) GetTypeDescription() *TypeDescription {
	if x != nil {
		return x.TypeDescription
	}
	return nil
}

var File_ast_member_access_proto protoreflect.FileDescriptor

var file_ast_member_access_proto_rawDesc = []byte{
	,
}

var (
	file_ast_member_access_proto_rawDescOnce sync.Once
	file_ast_member_access_proto_rawDescData = file_ast_member_access_proto_rawDesc
)

func file_ast_member_access_proto_rawDescGZIP() []byte {
	file_ast_member_access_proto_rawDescOnce.Do(func() {
		file_ast_member_access_proto_rawDescData = protoimpl.X.CompressGZIP(file_ast_member_access_proto_rawDescData)
	})
	return file_ast_member_access_proto_rawDescData
}

var file_ast_member_access_proto_msgTypes = make([]protoimpl.MessageInfo, 1)
var file_ast_member_access_proto_goTypes = []interface{}{
	(*MemberAccessExpression)(nil), // 0: txpull.v1.ast.MemberAccessExpression
	(NodeType)(0),                  // 1: txpull.v1.ast.NodeType
	(*Src)(nil),                    // 2: txpull.v1.ast.Src
	(*v3.TypedStruct)(nil),         // 3: xds.type.v3.TypedStruct
	(*TypeDescription)(nil),        // 4: txpull.v1.ast.TypeDescription
}
var file_ast_member_access_proto_depIdxs = []int32{
	1, // 0: txpull.v1.ast.MemberAccessExpression.node_type:type_name -> txpull.v1.ast.NodeType
	2, // 1: txpull.v1.ast.MemberAccessExpression.src:type_name -> txpull.v1.ast.Src
	3, // 2: txpull.v1.ast.MemberAccessExpression.expression:type_name -> xds.type.v3.TypedStruct
	4, // 3: txpull.v1.ast.MemberAccessExpression.argument_types:type_name -> txpull.v1.ast.TypeDescription
	4, // 4: txpull.v1.ast.MemberAccessExpression.type_description:type_name -> txpull.v1.ast.TypeDescription
	5, // [5:5] is the sub-list for method output_type
	5, // [5:5] is the sub-list for method input_type
	5, // [5:5] is the sub-list for extension type_name
	5, // [5:5] is the sub-list for extension extendee
	0, // [0:5] is the sub-list for field type_name
}

func init() { file_ast_member_access_proto_init() }
func file_ast_member_access_proto_init() {
	if File_ast_member_access_proto != nil {
		return
	}
	file_ast_types_proto_init()
	file_ast_src_proto_init()
	file_ast_type_name_proto_init()
	if !protoimpl.UnsafeEnabled {
		file_ast_member_access_proto_msgTypes[0].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*MemberAccessExpression); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_ast_member_access_proto_rawDesc,
			NumEnums:      0,
			NumMessages:   1,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_ast_member_access_proto_goTypes,
		DependencyIndexes: file_ast_member_access_proto_depIdxs,
		MessageInfos:      file_ast_member_access_proto_msgTypes,
	}.Build()
	File_ast_member_access_proto = out.File
	file_ast_member_access_proto_rawDesc = nil
	file_ast_member_access_proto_goTypes = nil
	file_ast_member_access_proto_depIdxs = nil
}
