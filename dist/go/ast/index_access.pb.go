// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.26.0
// 	protoc        v4.23.4
// source: ast/index_access.proto

package ast_pb

import (
	v3 "github.com/cncf/xds/go/xds/type/v3"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type IndexAccess struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Id                    int64              `protobuf:"varint,1,opt,name=id,proto3" json:"id,omitempty"`
	NodeType              NodeType           `protobuf:"varint,2,opt,name=node_type,json=nodeType,proto3,enum=txpull.v1.ast.NodeType" json:"node_type,omitempty"`
	Src                   *Src               `protobuf:"bytes,3,opt,name=src,proto3" json:"src,omitempty"`
	IndexExpression       *v3.TypedStruct    `protobuf:"bytes,4,opt,name=index_expression,json=indexExpression,proto3" json:"index_expression,omitempty"`
	BaseExpression        *v3.TypedStruct    `protobuf:"bytes,5,opt,name=base_expression,json=baseExpression,proto3" json:"base_expression,omitempty"`
	TypeDescriptions      []*TypeDescription `protobuf:"bytes,6,rep,name=type_descriptions,json=typeDescriptions,proto3" json:"type_descriptions,omitempty"`
	ReferencedDeclaration int64              `protobuf:"varint,7,opt,name=referenced_declaration,json=referencedDeclaration,proto3" json:"referenced_declaration,omitempty"`
	TypeDescription       *TypeDescription   `protobuf:"bytes,8,opt,name=type_description,json=typeDescription,proto3" json:"type_description,omitempty"`
}

func (x *IndexAccess) Reset() {
	*x = IndexAccess{}
	if protoimpl.UnsafeEnabled {
		mi := &file_ast_index_access_proto_msgTypes[0]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *IndexAccess) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*IndexAccess) ProtoMessage() {}

func (x *IndexAccess) ProtoReflect() protoreflect.Message {
	mi := &file_ast_index_access_proto_msgTypes[0]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use IndexAccess.ProtoReflect.Descriptor instead.
func (*IndexAccess) Descriptor() ([]byte, []int) {
	return file_ast_index_access_proto_rawDescGZIP(), []int{0}
}

func (x *IndexAccess) GetId() int64 {
	if x != nil {
		return x.Id
	}
	return 0
}

func (x *IndexAccess) GetNodeType() NodeType {
	if x != nil {
		return x.NodeType
	}
	return NodeType_NT_DEFAULT
}

func (x *IndexAccess) GetSrc() *Src {
	if x != nil {
		return x.Src
	}
	return nil
}

func (x *IndexAccess) GetIndexExpression() *v3.TypedStruct {
	if x != nil {
		return x.IndexExpression
	}
	return nil
}

func (x *IndexAccess) GetBaseExpression() *v3.TypedStruct {
	if x != nil {
		return x.BaseExpression
	}
	return nil
}

func (x *IndexAccess) GetTypeDescriptions() []*TypeDescription {
	if x != nil {
		return x.TypeDescriptions
	}
	return nil
}

func (x *IndexAccess) GetReferencedDeclaration() int64 {
	if x != nil {
		return x.ReferencedDeclaration
	}
	return 0
}

func (x *IndexAccess) GetTypeDescription() *TypeDescription {
	if x != nil {
		return x.TypeDescription
	}
	return nil
}

var File_ast_index_access_proto protoreflect.FileDescriptor

var file_ast_index_access_proto_rawDesc = []byte{
	,
}

var (
	file_ast_index_access_proto_rawDescOnce sync.Once
	file_ast_index_access_proto_rawDescData = file_ast_index_access_proto_rawDesc
)

func file_ast_index_access_proto_rawDescGZIP() []byte {
	file_ast_index_access_proto_rawDescOnce.Do(func() {
		file_ast_index_access_proto_rawDescData = protoimpl.X.CompressGZIP(file_ast_index_access_proto_rawDescData)
	})
	return file_ast_index_access_proto_rawDescData
}

var file_ast_index_access_proto_msgTypes = make([]protoimpl.MessageInfo, 1)
var file_ast_index_access_proto_goTypes = []interface{}{
	(*IndexAccess)(nil),     // 0: txpull.v1.ast.IndexAccess
	(NodeType)(0),           // 1: txpull.v1.ast.NodeType
	(*Src)(nil),             // 2: txpull.v1.ast.Src
	(*v3.TypedStruct)(nil),  // 3: xds.type.v3.TypedStruct
	(*TypeDescription)(nil), // 4: txpull.v1.ast.TypeDescription
}
var file_ast_index_access_proto_depIdxs = []int32{
	1, // 0: txpull.v1.ast.IndexAccess.node_type:type_name -> txpull.v1.ast.NodeType
	2, // 1: txpull.v1.ast.IndexAccess.src:type_name -> txpull.v1.ast.Src
	3, // 2: txpull.v1.ast.IndexAccess.index_expression:type_name -> xds.type.v3.TypedStruct
	3, // 3: txpull.v1.ast.IndexAccess.base_expression:type_name -> xds.type.v3.TypedStruct
	4, // 4: txpull.v1.ast.IndexAccess.type_descriptions:type_name -> txpull.v1.ast.TypeDescription
	4, // 5: txpull.v1.ast.IndexAccess.type_description:type_name -> txpull.v1.ast.TypeDescription
	6, // [6:6] is the sub-list for method output_type
	6, // [6:6] is the sub-list for method input_type
	6, // [6:6] is the sub-list for extension type_name
	6, // [6:6] is the sub-list for extension extendee
	0, // [0:6] is the sub-list for field type_name
}

func init() { file_ast_index_access_proto_init() }
func file_ast_index_access_proto_init() {
	if File_ast_index_access_proto != nil {
		return
	}
	file_ast_types_proto_init()
	file_ast_src_proto_init()
	file_ast_type_name_proto_init()
	if !protoimpl.UnsafeEnabled {
		file_ast_index_access_proto_msgTypes[0].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*IndexAccess); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_ast_index_access_proto_rawDesc,
			NumEnums:      0,
			NumMessages:   1,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_ast_index_access_proto_goTypes,
		DependencyIndexes: file_ast_index_access_proto_depIdxs,
		MessageInfos:      file_ast_index_access_proto_msgTypes,
	}.Build()
	File_ast_index_access_proto = out.File
	file_ast_index_access_proto_rawDesc = nil
	file_ast_index_access_proto_goTypes = nil
	file_ast_index_access_proto_depIdxs = nil
}
